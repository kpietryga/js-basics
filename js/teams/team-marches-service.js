...
export const teamMatchesService = (team) => {
    ...
    /**
     * Calculates statistics related to goals.
     *
     * @returns {Object} - An object containing the average number of minutes between goals scored and conceded.
     */
    const calculateGoalStatistics = () => {
        const {teamGoals, opponentGoals} = getGoalBalance();

        const totalMinutes = matches.length * 90;

        const minutesPerGoalScored = totalMinutes / teamGoals;
        const minutesPerGoalLosses = totalMinutes / opponentGoals;

        return {
            minutesPerGoalScored,
            minutesPerGoalLosses
        };
    };

    return {
        getPoints,
        getLongestHomeUnbeatenStreak,
        getGoalBalance,
        getAverageAttendance,
        getHomeAwayWinBalance,
        getBiggestWinAndLoss,
